...
class zhihu:
    ...
    def get_index(self):
        ...
        response = s.get(url='https://www.zhihu.com/',headers =self.headers )
    ...
    def crawlzhihu(self):
        while True:
            try:
                ...
                captcha_data = s.get(captcha_url, headers=self.headers).content
                text = self.captcha(captcha_data)
                post_data = {
                    "_xsrf":_xsrf,
                ...
                response_text = s.post(url=post_url,data=post_data,headers=self.headers)
                # print(response_text.text.encode('utf-8'))
                get_response = s.get(url='https://www.zhihu.com/', headers=self.headers)
